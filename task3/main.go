...
func SlowSearch(out io.Writer) {
	...
	r := regexp.MustCompile("@") //Тут наверное надо как-то сделать прекомпиляицю регулярки
	seenBrowsers := []string{}
	uniqueBrowsers := 0
	foundUsers := ""
...
func main() {
	...
	if err != nil {
		log.Fatal("не удалось создать профиль CPU:", err)
	}
	defer f.Close()

	// Начинаем запись профилирования
